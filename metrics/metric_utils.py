...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, **stats_kwargs):
    ...
    def run_generator(z, c,image):
        img = G(z=z, c=c, image=image, **opts.G_kwargs)
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            image = torch.randn([batch_gen, dataset.image_shape[0], dataset.image_shape[1], dataset.image_shape[2]], device=opts.device)
            images.append(run_generator(z, c, image))

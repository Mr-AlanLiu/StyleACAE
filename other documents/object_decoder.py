...
class CapsuleLayer(nn.Module):
    ...
    def forward(self, feature, parent_transform=None, parent_presence=None):
        ...
        Returns:
          A bunch of stuff.
        """
        #device = torch.divice('cuda' if torch.cuda.is_available() else 'cpu' )
        ...
        #caps_feature_list = caps_feature_list.cuda()
        caps_param_list = [self.mlps[i] (caps_feature_list[i]) 
                           for i in range(self.n_caps)] # [(B, D)] * O self.n_caps
        #caps_param_list = caps_param_list.gpu
        #caps_param_list = torch.nn.ModuleList(caps_param_list)                  
        del feature, caps_feature_list
        raw_caps_param = torch.stack(caps_param_list, 1)  # (B, O, D)
        del caps_param_list

        if self.caps_dropout_rate == 0.0:
            caps_exist = torch.ones(batch_size, 144, 1)  # (B, O, 1)  self.n_caps
        else:
            pmf = Bernoulli(1. - self.caps_dropout_rate)
            caps_exist = pmf.sample((batch_size, 144, 1))  # (B, O, 1) self.n_caps
        ...
        all_param_list = [self.caps_mlps[i](caps_eparam_list[i])
                          for i in range(144)]  # [(B, A)] * O  self.n_caps
        del caps_eparam_list
        all_param = torch.stack(all_param_list, 1)  # (B, O, A)
        del all_param_list
        all_param_split_list = torch.split(all_param, self.splits, -1)
        result = [t.view(batch_size, 144, *s)
                  for (t, s) in zip(all_param_split_list, self.output_shapes)]  #self.n_caps
        del all_param
...
class CapsuleLikelihood:
    ...
    def __call__(self, x, presence=None):  # (B, M, P), (B, M)
        ...
        dummy_vote = self.dummy_vote.repeat(batch_size, 1, 1, 1).to(device)  # (B, 1, M, P)
        dummy_presence = torch.zeros([batch_size, 1, n_input_points], device=device)

        votes = torch.cat((self.vote, dummy_vote), 1).to(device) # (B, O+1, M, P)
...
class CapsuleObjectDecoder(nn.Module):
    def __init__(self, capsule_layer: CapsuleLayer):
        ...
        self.dummy_vote = nn.Parameter(
            torch.zeros(1, 1, 32, capsule_layer.n_transform_params),
            requires_grad=True
        )

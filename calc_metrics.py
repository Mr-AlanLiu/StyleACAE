...
import training.training_loop #import training_loop #import training_loop

#----------------------------------------------------------------------------

def subprocess_fn(rank, args, temp_dir,data):
    ...
    #training_set = dnnlib.util.construct_class_by_name(**training_loop.training_set_kwargs) # subclass of training.dataset.Dataset
    #training_set1 = training.training_loop.training_loop(training_set_kwargs)
    #training_set = dnnlib.util.construct_class_by_name(**training_set1.training_set_kwargs)
    ...
    if rank == 0 and args.verbose:
        ...
        #args.training_set = dnnlib.util.construct_class_by_name(**training_set_kwargs)
        args.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data)
        training_set = dnnlib.util.construct_class_by_name(**args.dataset_kwargs)
        image = torch.empty([ 1, training_set.image_shape[0], training_set.image_shape[1], training_set.image_shape[2]], device=device)
        #image= torch.empty([],device=device)
        #image = torch.empty()
        misc.print_module_summary(G, [z, c, image])
...
def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, verbose):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        if args.num_gpus == 1:
            subprocess_fn(rank=0, args=args, temp_dir=temp_dir,data=data)
        else:
